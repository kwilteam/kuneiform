...
func TestParse_valid_syntax(t *testing.T) {
	tests := []struct {
		...
		// blob column
		{"table with blob column", `database td1; table tt1 { tc1 blob}`,
			genOneTableOneCol(schema.ColBlob),
		},
		{"table with blob column with all attrs",
			`database td1; table tt1 { tc1 blob minlen(3) maxlen(30) default(3) notnull primary unique }`,
			genOneTableOneColWithAttrs(schema.ColBlob,
				schema.Attribute{Type: schema.AttrMinLength, Value: "3"},
				schema.Attribute{Type: schema.AttrMaxLength, Value: "30"},
				schema.Attribute{Type: schema.AttrDefault, Value: "3"},
				schema.Attribute{Type: schema.AttrNotNull},
